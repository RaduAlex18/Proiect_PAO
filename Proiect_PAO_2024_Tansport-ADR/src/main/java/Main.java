...
import Domain.Clienti.Client1;
import Domain.Cursa.Cursa;
import Domain.Marfa.Marfa;
import Domain.Proprietati.Teren;
import Repository.CamionRepository;
import Repository.CisternaRepository;
import Repository.SoferRepository;
import Repository.TahografRepository;
import Service.FlotaService;
...
public class Main {
    public static void main(String[] args) {
        ...
        SoferRepository soferRepository = new SoferRepository();
        CisternaRepository cisternaRepository = new CisternaRepository();
        TahografRepository tahografRepository = new TahografRepository();

        setupData.createCisternaTable();
        setupData.createCamionTable();
        setupData.createSoferTable();
        setupData.createTahografTable();

        // Testarea metodelor CRUD pentru Camion
        System.out.println("Testarea metodelor CRUD pentru Camion:");
        camionRepository.createCamion("Volvo", "AB123CD", 2018, 50000, "Red", 4000);
        camionRepository.createCamion("Scania", "EF456GH", 2019, 75000, "Blue", 4500);
        camionRepository.updateCamion(1, "MAN", "IJ789KL", 2020, 100000, "White", 5000);
        camionRepository.deleteCamion(2);
        camionRepository.showAllCamion();

        // Testarea metodelor CRUD pentru Sofer
        System.out.println("Testarea metodelor CRUD pentru Sofer:");
        soferRepository.createSofer("Ion", "Popescu", 5, 100000, 1);
        soferRepository.createSofer("Mihai", "Ionescu", 3, 50000, 1);
        soferRepository.updateSofer(1, "Ion", "Marin", 7, 120000, 1);
        soferRepository.deleteSofer(2);
        soferRepository.showAllSofer();

        // Testarea metodelor CRUD pentru Cisterna
        System.out.println("Testarea metodelor CRUD pentru Cisterna:");
        cisternaRepository.createCisterna(10, "MN123OP", 2015, 80000, "Mercedes");
        cisternaRepository.createCisterna(12, "QR456ST", 2016, 60000, "DAF");
        cisternaRepository.updateCisterna(1, 15, "UV789WX", 2017, 90000, "Iveco");
        cisternaRepository.deleteCisterna(2);
        cisternaRepository.showAllCisterna();

        // Testarea metodelor CRUD pentru Tahograf
        System.out.println("Testarea metodelor CRUD pentru Tahograf:");
        tahografRepository.createTahograf(1, java.sql.Date.valueOf("2024-05-27"), 30000, 500);
        tahografRepository.updateTahograf(1, 1, java.sql.Date.valueOf("2024-05-27"), 50000, 800);
        tahografRepository.showAllTahograf();

        cisternaRepository.createCisternaObject(cisterna1);
